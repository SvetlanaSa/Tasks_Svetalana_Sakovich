...
public class CreditClassDAO implements ICreditCardDAO {
    ...
    public List<Credit_Card> getAllCredit() throws SQLException {
        ...
        try {
            ...
            while (result.next()){
                credit_Card.add(new Credit_Card(result.getInt("account_id"),result.getDouble("cash_amount")));
            }
    ...
    @Override
    public Credit_Card getCredit_Card(int Credit_Card_ID) throws SQLException {
        Connection connection = connectionFactory.getConnection();
        ...
        try {
            ...
            mystm.setInt(1,Credit_Card_ID);
            result = mystm.executeQuery();

            while (result.next()) {
                credit_Card = new  Credit_Card(result.getInt("account_id"),result.getDouble("cash_amount"));
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
    ...
    public void updateCredit_Card(Credit_Card credit_Card) throws SQLException {
        ...
        try {
            ...
            mystm.setLong(1,credit_Card.getCredit_Card_ID());
            mystm.setInt(2, credit_Card.getAccount_ID());
            mystm.setDouble(3,credit_Card.getCash_amount());
            mystm.execute();
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    @Override
    public void deleteCredit_Card(Credit_Card credit_Card) throws SQLException {
        PreparedStatement mystm = null;
        Connection connection = connectionFactory.getConnection();
        try {
